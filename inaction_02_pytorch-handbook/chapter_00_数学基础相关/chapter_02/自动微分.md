...
对应代码：`01_autograd_automatic_differention.py`
...
8. 向量/张量的反向传播：为什么需要传入 `gradient` 参数（含雅可比矩阵原理图、动态计算图例子）
...
![正向传播与反向传播示意图](forward_backward_diagram.png)
...
![三层链式法则反向传播动图](chain3_grad_animation.gif)
...
上面 6.2 的例子里 `x`、`y` 各自是**一个独立的标量变量**。但实际写代码时更常见的是像下面这样的场景（对应 `01_autograd_automatic_differention.py`）：
...
![逐元素乘法反向传播动图](elementwise_mul_grad_animation.gif)
...
![雅可比矩阵与向量-雅可比积示意图](jacobian_diagram.png)
